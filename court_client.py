...
class CourtClient:
    ...
    async def connect(self):
        """Connect to the Spinstat Backend WebSocket"""
        # Determine protocol based on host
        protocol = "wss" if "fly.dev" in self.config['server_host'] or self.config.get('use_ssl', False) else "ws"
        
        # Build WebSocket URL
        if self.config['server_port'] and self.config['server_port'] not in ['80', '443']:
            uri = f"{protocol}://{self.config['server_host']}:{self.config['server_port']}/court-ws"
        else:
    ...
    async def heartbeat_loop(self):
        ...
        while self.running and self.ws:
            try:
                ...
                # Check if connection is still alive using the state property
...
def load_config():
    ...
    config = {
        ...
        'court_number': os.getenv('COURT_NUMBER', ''),
        'club_name': os.getenv('CLUB_NAME', ''),
        'location': os.getenv('LOCATION', ''),
        'court_type': os.getenv('COURT_TYPE', '')
    }
...
async def main():
    ...
    if config.get('club_name'):
        print(f"🏢 Club: {config['club_name']}")
    if config.get('court_number'):
        print(f"🎾 Court Number: {config['court_number']}")
